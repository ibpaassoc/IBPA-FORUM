  ...
  return (
    ...
    >
      ...
      <div className="mt-[var(--space-md)]">
        <div className="flex items-center justify-between gap-3 text-xs uppercase tracking-[0.16em] text-[var(--color-hover)]">
          <span>{progressLabel}</span>
          <span>{Math.max(0, Math.min(100, progressValue))}%</span>
        </div>
        <div className="mt-2 h-2 rounded-full bg-[var(--color-mist)]">
          <div
            className="h-2 rounded-full bg-[var(--color-hover)] transition-all duration-500"
