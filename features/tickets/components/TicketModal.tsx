...
import { controlSpring, dialogVariants, overlayVariants, riseVariants } from "@/features/tickets/motion";
...
export default function TicketModal({ isOpen, onClose }: TicketModalProps) {
  ...
  const modal = (
    // reducedMotion="user" turns off transform and layout animations tree-wide
    // — including inside TicketForm — so individual components only have to
    // opt out of the motion that framer cannot neutralise on its own (hover).
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="no-scrollbar relative max-h-[96dvh] w-full max-w-[1180px] overflow-y-auto overscroll-contain rounded-[26px] border border-white/85 bg-[linear-gradient(145deg,rgba(255,255,255,0.98),rgba(238,248,253,0.96))] font-[var(--font-ui-family)] shadow-[0_40px_110px_rgba(3,18,38,0.38)] sm:max-h-[92vh] sm:rounded-[30px]"
            ...
            >
              ...
              {/* Not a motion element: TicketForm owns the body's own cascade,
                  and a second animated wrapper here would double it up. */}
