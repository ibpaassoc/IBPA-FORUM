...
export function mapApplicationRow(app: ApplicationRecord): SheetValues[number] {
  ...
  return [
    ...
    app.category.name,
    nominationLabel,
    member ? "IBPA Member" : "Non-member",
    yesNo(member),
    app.membershipNumber ?? "",
    formatUsd(app.amount),
    formatUsd(applicationAmountPaidCents(app)),
    humanizeEnum(app.paymentStatus),
    humanizeEnum(app.status),
    app.checkedInAt ? formatDateTime(app.checkedInAt) : "No",
...
export function mapJuryRow(jury: JuryRecord): SheetValues[number] {
  return [
    ...
    humanizeEnum(jury.paymentStatus),
    humanizeEnum(jury.status),
    jury.checkedInAt ? formatDateTime(jury.checkedInAt) : "No",
...
export function mapTicketRow(ticket: TicketRecord): SheetValues[number] {
  ...
  return [
    ...
    ticket.stripePaymentIntentId ?? ticket.stripeSessionId ?? "",
    ticket.secureToken,
