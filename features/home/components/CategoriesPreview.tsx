    ...
    <section className="bg-(--color-white)">
      <div className="mx-auto max-w-(--content-width) px-(--page-gutter) page-section-pad">
        <div className="flex flex-col gap-(--space-md) md:flex-row md:items-end md:justify-between">
          ...
          <Link
            href="/categories"
            className="font-(--font-sans) text-[clamp(0.72rem,1vw,0.85rem)] uppercase tracking-widest text-(--color-navy) transition hover:text-(--color-hover)"
          >
        ...
        <div className="mt-(--space-xl) grid gap-(--space-sm) sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {categories.map((category, index) => (
            <Link
              key={category}
              href="/categories"
              className="page-card group bg-(--color-off-white) p-(--space-md) transition hover:border-(--color-hover) hover:bg-(--color-navy)"
            >
